...
//    WebUtils.toHttp(response).sendRedirect("http://localhost:1002/login");
    ...
    private boolean validateClient(ServletRequest request, ServletResponse response) {
        ...
        if (StringUtils.isNotBlank(code)) {
            // HttpPost去校验code
            try {
//        StringBuffer sso_server_url = new StringBuffer(PropertiesFileUtil.getInstance("zheng-upms-client").get
// ("zheng.upms.sso.server.url"));
...
//            _LOGGER.debug("当前code={}，对应的注册系统个数：{}个", code, RedisUtil.getJedis().scard(AUTHC_CLIENT_SESSION_IDS +
// "_" + code));
//            // 移除url中的token参数
//            String backUrl = RequestParameterUtil.getParameterWithOutCode(WebUtils.toHttp(request));
//            // 返回请求资源

...
public class AliyunOssUtil {
    ...
    public static List<String> deleteObjects(List<String> keys) {
        ...
        DeleteObjectsResult deleteObjectsResult = ossClient.deleteObjects(new DeleteObjectsRequest
                (TINGDUODUO_BUCKET_NAME).withKeys
                (keys));
        // 返回删除成功的文件列表
     ...
     * @return String
     */
    public static String getSignUrl(String objectName) {
        final char beginStr = objectName.charAt(0);
        if (Constants.OSS_NOT_START == beginStr) {
            objectName = objectName.substring(1, objectName.length());
        }
        ...
        Date expiration = new Date(System.currentTimeMillis() + 1800 * 1000);
        // 生成已GET方法访问的签名URL
        URL url = ossClient.generatePresignedUrl(TINGDUODUO_BUCKET_NAME, objectName, expiration);
        // 关闭OSSClient。
        ossClient.shutdown();
        return url.toString();
    }

(function($) {
    $.fn.upload = function(look_call_back){
        ...
        config = {
            ...
            stype : _this.attr("data-stype") ? _this.attr("data-stype") : "",
        };
        ...
        _this.append($add.attr("data-num",config.num).attr("data-type",config.type))
            ...
            {
                var url = $(e.currentTarget).parent("li.success").css("background-image");
                var str = url.match(/^.+\/(\w+\.\w+)/i);
                var fileName = str[1];
                var deleteUrl = "/slideshow/delete/" + fileName;
            ...
            for (var item in fileObj) {
                ...
                { 
                    var rs = addfile(fileObj[item],function(data,item,file){
                        ...
                        formData.append("type", config.stype);
